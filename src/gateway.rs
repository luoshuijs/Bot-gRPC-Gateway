...
use crate::gateway_proto::{
    ...
    Message, RegisterResponse, ServiceInfo, HealthCheckRequest, HealthCheckResponse
};
...
impl GatewayService for GatewayServiceImpl {
    ...
    async fn forward(&self, request: Request<Message>) -> Result<Response<Message>, Status> {
        ...
        tracing::debug!(
            ...
            msg.source_service, msg.cmd, msg.user_id
        );
    ...
    // 服务注册
    async fn register(&self, request: Request<ServiceInfo>) -> Result<Response<RegisterResponse>, Status> {
        let info = request.into_inner();

        tracing::info!(
            "Registering service: {} ({}), addr: {}",
            info.service_name, info.service_id, info.service_addr
        );
    ...
    async fn stream(&self, request: Request<tonic::Streaming<Message>>) -> Result<Response<Self::StreamStream>, Status> {
        ...
        tokio::spawn(async move {
            while let Some(result) = in_stream.next().await {
                match result {
                    Ok(msg) => {
                        tracing::debug!("Stream message from {}: cmd={}", msg.source_service, msg.cmd);
    ...
    async fn health_check(&self, request: Request<HealthCheckRequest>) -> Result<Response<HealthCheckResponse>, Status> {
        ...
        // 获取服务健康状态
        let is_healthy = self.router.health_status
            .get(&service_id)
            .map(|status| *status)
            .unwrap_or(false);
